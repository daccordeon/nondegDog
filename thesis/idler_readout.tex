...
Finally, for gravitational-wave detection, I showed that the most promising application of idler readout at present~\footnote{At least, among those that I considered.} is to use incoherently combined signal and idler readouts for broadband detection from 0.1--4~kHz\jam{(check)}, possibly with variational readout of the idler to squeeze the radiation-pressure noise. % but that kilohertz sensitivity from 1--4~kHz is improved more by signal readout. 
