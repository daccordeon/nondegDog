...
\begin{figure}
	\centering
	% \includegraphics[width=\textwidth]{}
	\caption{Degenerate OPO.}
	\label{fig:dOPO_config}
\end{figure}

A degenerate optical parametric oscillator (OPO) consists of a squeezer crystal inside of an optical cavity, operating in the degenerate regime (i.e.\ producing two squeezed photons at the same frequency for each pump photon annihilated), as shown in Fig.~\ref{fig:dOPO_config}. This configuration is the essential squeezing set-up and is well-studied in the literature~\cite{}. I examine this configuration here for two main purposes: (1) to show that the photons produced in parametric down-conversion are squeezed and (2) to demonstrate the analytic, Hamiltonian model that I will use throughout the rest of this thesis (this model exists widely in the literature~\cite{}).  
The pump laser is shown incident orthogonally on the crystal to the beam path, this is only somewhat realistic~\cite{} and spatial aspects of the configuration are not included in the model. Making the single-mode approximation, I assume that the pump laser down-converts into the cavity mode. Vacuum at the cavity mode frequency enters the cavity through the main, readout port (connected to the photodetector) and through a single loss port, shown in Fig.~\ref{fig:dOPO_config}. 
% OPO = squeezed cavity with no seed light
In the literature, a distinction is sometimes made between when the squeezed field is ``seeded'' or not, i.e.\ whether the degenerate OPO produces squeezed vacuum or squeezed light, and in the latter case the OPO is instead called an optical parametric amplifier (OPA). But since I am interested in the quantum noise response of this configuration, i.e.\ how vacuum noise appears in the final measurement, I do not need to worry about this distinction. % is this even necessary to bring up then?
The dynamics of the pump laser can then be ignored as I approximate its large coherent amplitude as being a reservoir of photons in comparison to the empty cavity mode, this ignores the effect of pump depletion~\cite{} and is a valid approximation~\cite{} in the parameter regime that I stick to -- more on this later. 
