...
\end{equation} 
Then the combined variance is, e.g.\ for $\psi_2=\pi/4,\psi_1=\psi_0=0$,
...
\begin{figure}
	...
	\caption{\jam{(Unify 10 dB definition with Chapter 5. Add frequency-dependent noise ellipses to external squeezing? Look up QRPN formula.)} Quantum noise response of the detector shown in Fig.~\ref{fig:DRFPMI} to quantum radiation-pressure noise, shot noise, and total quantum noise.	I use the parameters of LIGO~Voyager~\cite{} from Section~\ref{sec:dIS_results}. At kilohertz, shot noise is the dominant source of quantum noise. The point where the two sources of quantum noise balance is shown and is related to the Standard Quantum Limit~\cite{} which I will not discuss it here because it is weaker than the Mizuno limit and can also be beaten by squeezing~\cite{}.
