...
\begin{figure}[ht]
    ...
    I use large idler loss ($T_{l,c}=0.1$) to show the trajectories more clearly, but if it is realistic (e.g.\ $T_{l,c}=1000\text{ppm}$), then (b) and (c) move closer to (a)\jam{(check)}. I use the parameters in Tab.~\ref{tab:signal_RO_parameters} with zero signal loss ($T_{l,b}$).}
