...
Throughout the rest of this thesis, I will examine the results using the parameter set in Tab.~\ref{tab:signal_RO_parameters} which is based on LIGO~Voyager~\cite{}. As discussed in Section~\ref{sec:dIS_results}, the choice of parameter set can bias the analysis of a configuration and I mention, where possible, the effects of varying the parameters, e.g.\ the readout rates which have a large variation in the literature~\cite{}. These parameters have not been optimised for the sensitivity which I leave to future work. % parameter set. % from Section~\ref{sec:dIS_results}, in particular, I will use the  unless stated otherwise.
