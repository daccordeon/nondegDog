...
\begin{figure}
	...
	\caption{\jam{(Purpose: show limits 1/2)} Nondegenerate internal squeezing sensitivity in the lossless limit, with and without radiation pressure noise, compared to the lossless optomechanical analogue from Fig.~\ref{fig:sWLC_sensitivity}. This is shown for 500~Hz signal readout rate and no idler readout rate \jam{(check)}. The sensitivity converges as shown, as do the signal and noise responses. Compared to the lossy sensitivity without radiation-pressure noise in Fig.~\ref{fig:nIS_general_sens}, the signal DC response is improved which means that the losses determine the bandwidth of the squeezer's signal amplification \jam{(check)}.}
