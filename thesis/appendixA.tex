...
\section{Appendix A.1: Singularity threshold for degenerate internal squeezing}
% \addcontentsline{toc}{section}{Appendix A.1: Singularity threshold for degenerate internal squeezing} 
\label{app:dIS_singularity_thr}
\jam{(Fix title)} %\appendix?

\renewcommand{\thefigure}{A\arabic{figure}}
\setcounter{figure}{0}
\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{dIS_threshold_traj.pdf}
    \caption{\jam{(Purpose: show a complication with singularity threshold. Quantify what arm loss is required for a significant difference between singularity and extremum. Fix figure numbering. Changing Tla with $Tlb=0$)} \jam{(Fix caption. Explain traj labels. Explain why it doesn't reach (d))} Degenerate internal squeezing trajectory of singularities of the anti-squeezed quadrature of the noise, as well as of the extrema of the squeezed quadrature, in $(\Omega, \chi)$ space as the arm loss is changed and the signal loss is zero. I assume that the extrema are minima because of the shape of the squeezed curves in Figs.~\ref{fig:dOPO_variances}~\ref{fig:dIS_noise_budget}. \jam{(could just check this, future work?)} The singularities and extrema both achieve the same limits but diverge at high arm losses \jam{(looks like they diverge immediately, explain that effect is small on the logarithmic frequency scale, maybe plot it?)}. \jam{(rest of explanation moved into text, does it still make sense?)}}
...
% \end{figure}
However, these values of $(\Omega_\text{thr},\chi_\text{thr})$, which are where the anti-squeezed quadrature is divergent, do not necessarily correspond to where the squeezed quadrature has the minimum value, i.e.\ where the peak is the most squeezed towards zero. This is unlike the degenerate OPO for which a simple calculation shows that the squeezed quadrature is minimised on threshold \jam{(show proof?)}.
If the minima~\footnote{Since the zeros of the squeezed quadrature are not robust to losses, as shown for the OPO in Fig.~\ref{fig:dOPO_variances}, I consider the minima instead.} of the squeezed quadrature were used to define threshold, then their trajectories in (real) $(\Omega,\chi)$ space would be as shown in Fig.~\ref{fig:dIS_threshold_traj}. Although they achieve the same limits, these trajectories are not the same as the singularity trajectories, e.g.\ they merge at a different $\chi$ value.
...
Although I do not understand why this difference occurs \jam{(check this)}, two possible explanations are that it comes from the different DC behaviour of the different quadratures of the limiting lossy degenerate OPO in Fig.~\ref{fig:dOPO_variances} or that the approximation to the sloshing frequency in Section~\ref{sec:dIS_model} breaks down in the high arm loss limit~\footnote{In which case, this model would not represent the physical system in that limit.}~\cite{}. This might be understood better once singularity threshold is verified against a pump-depletion model in future work.
