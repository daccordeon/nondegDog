...
% squeezing ellipse and signal arrow plot (+ show the effect of optical loss: detection and intracavity)
	\caption{\jam{(Don't use coupled-cavity approximation here (make it look like an IFO), simplify or cut loss ports, just say ETM. Extend SRC to ITMs.)} Degenerate internal squeezing configuration (top panel) with the single-mode approximation, and the squeezer's effect on the measured signal and noise (bottom panel) using the noise ellipse and signal arrow representation~\cite{} where the height of the arrow represents the signal response.
...
\jam{(Do I need to talk about stability here? ``Move to an appendix'' - VBA)}
