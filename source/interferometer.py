...
from dIS_Ripoll import ASDSh_dIS, ASDSx_dIS, sigTabs_dIS
...
def readout_rate(tRT, T):
    """tRT: round-trip time, T: transmission; returns a readout_rate in 2 pi Hz? TODO: check"""
    return -0.5 / tRT * np.log(1 - T)
...
class IFO(object):
    def __init__(
        ...
        self.mu = self.M / 4
        self.rho = (sqrt(2) * ((self.B**2) * ((self.L_arm**-2.0) * hbar))) / self.mu
        ...
        if T_SRM is not None:
            ...
            if gbR is None and gcR is None:
                ...
                # Tsrm is for signal, calculate Tsrm_c manually from: 1-np.exp(2*pi*self.gcR/(-0.5/self.tRT_SRC)) TODO: check this, why the factor of 2 pi if gcR already angular?
                self.L_SRC = c * self.tRT_SRC / 2

            if ws is not None:
                self.ws = ws
                self.T_ITM = (
                    (2 * self.ws / c) ** 2 * self.L_arm * self.L_SRC
                )  # Titm inferred from ws
            else:
                self.T_ITM = T_ITM
                self.ws = 0.5 * (c * (sqrt(((self.T_ITM / self.L_SRC) / self.L_arm))))
                
        # alternative: keeping Titm fixed and varying SRC parameters to achieve target gbR, gcR, and ws
        elif T_ITM is not None:
            self.T_ITM = T_ITM
            
            # setting ws and L_SRC
            if ws is not None:
                self.ws = ws
                # Lsrc inferred from given ws
                self.L_SRC = 1./((2 * self.ws / c) ** 2 * self.L_arm / self.T_ITM)  
            else:
            ...
            # setting gbR, gcR, and T_SRM
            if gbR is None and gcR is None:
                assert T_SRM is not None
                self.T_SRM = T_SRM
                self.gbR = readout_rate(self.tRT_SRC, self.T_SRM)
                # default to signal-idler symmetric at SRM
                self.gcR = self.gbR
            elif gbR is None and gcR is not None:
                assert T_SRM is not None
                self.T_SRM = T_SRM
                self.gbR = readout_rate(self.tRT_SRC, self.T_SRM)
                self.gcR = gcR
            elif gbR is not None and gcR is None:
                assert T_SRM is not None
                self.T_SRM = T_SRM
                self.gbR = gbR
                self.gcR = readout_rate(self.tRT_SRC, self.T_SRM)
                ...
                self.gbR = gbR
                # inferring T_SRM (signal mode transmittivity, TODO: Add T_SRM_c as an attribute.) from gbR
                self.T_SRM = 1 - np.exp(self.gbR/(-0.5 / self.tRT_SRC))
                assert gcR is not None
                self.gcR = gcR
        # TODO: propagate these added attributes
        self.T_SRM_b = self.T_SRM
        self.T_SRM_c = 1 - np.exp(self.gcR/(-0.5 / self.tRT_SRC))
...
----------------------""".format(
    ...
    def ga_fn(self, T_loss_a):
        return readout_rate(self.tRT_arm, T_loss_a)

    def gbtot_fn(self, T_loss_b):
        return self.gbR + readout_rate(self.tRT_SRC, T_loss_b)

    def gctot_fn(self, T_loss_c):
        return self.gcR + readout_rate(self.tRT_SRC, T_loss_c)
    ...
    def singularity_thr(self, T_loss_a, T_loss_b, T_loss_c):
        ...
        if T_loss_a != 0:
            thr = min(
                sqrt((gctot * (gbtot + ((self.ws**2) / ga)))),
                sqrt(
                    ((ga + gbtot) * (ga + (gctot + ((self.ws**2) / (gbtot + gctot)))))
                ),
            )
        else:
            thr = sqrt(
    ...
    def sensList_vs_freq(
        ...
        def sens_given_params(f):
            return self.ASDSh(
                f,
                *params,
                radiation_pressure_on=radiation_pressure_on,
                extSqzFactor=extSqzFactor,
                wm=wm,
                psi3=psi3
            )
    ...
    def plot_NSR_vs_freq(
        ...
        show_fig=True,
        fmt_List=None,
        radiation_pressure_List=None,
        extSqzFactor_List=None,
        wm_List=None,
        psi3_List=None,
        figsize=(6, 4),
        show_legend=True,
        ...
        for i, params in enumerate(paramsList):
            ...
            with Pool() as pool:  # check whether restarting the pool results in any speedu
                ...
                ax.loglog(
                    f_List,
                    pool.map(sens_given_params, f_List),
                    fmt_List[i],
                    label=labels[i],
                )
    ...
    def point_log_sens_given_angles(self, psi_angles, *params, **kwargs):
        ...
        **kwargs: Options passed to ASDSh, e.g. extSqzFactor."""
        return np.log10(self.ASDSh(*params, *psi_angles, **kwargs))

    def point_log_sens_given_angles_complex(self, psi_angles_complex, *params, **kwargs):
        """returns log10 of ASD of NSR to avoid underflow when optimising,
        psi_angles is a list of [psi0, psi1, psi2, psi3] to optimise over,
        params is fixed values of [f (the point), xRatio, T_loss_a, T_loss_b, T_loss_c, Rpd, pumpPhi]
        **kwargs: Options passed to ASDSh, e.g. extSqzFactor."""
        return np.log10(
            self.ASDSh(*params, *psi_angles_complex[:-1], psi3=psi_angles_complex[-1], **kwargs)
        )

    def save_idler_varRO(self, freq_tuple, losses1, file_tag, max_iter=5, skip_if_exists=False, **kwargs):
        ...
        **kwargs: Options passed to sensList_vs_freq"""
        filename = f"./optimal_angles/data_idler_(freq,psi1,varSens,fixedSens)--{file_tag}.npy"
        ...
        def point_log_sens_given_psi1(psi1_vec, *params):
            ...
            return self.point_log_sens_given_angles([0, psi1_vec[0], pi / 2], *params, **kwargs)
        ...
        def idler_varRO_given_freq(freq):
            ...
            result = dual_annealing(
                point_log_sens_given_psi1, bounds=bounds1, args=args0, maxiter=max_iter
            )
            return result.x

        psi1_List = np.array(p_map(idler_varRO_given_freq, freq_List))

        fixed_idler_sens = self.sensList_vs_freq(
            [xRatio0, *losses1, *idlerRO], freq_tuple, **kwargs
        )
        ...
        def sens_idler_varRO_given_freq(f):
            ...
            return self.ASDSh(f, *params0, 0, psi1_List[freq_index], pi / 2, **kwargs)
    ...
    def save_signal_varRO(self, freq_tuple, losses1, file_tag, max_iter=5, skip_if_exists=False, **kwargs):
        """saves a dataset of the variational readout of the signal mode
        freq_tuple=(fmin, fmax, fnum), losses1 is like losses0, file_tag for path
        **kwargs: Options passed to ASDSh, e.g. extSqzFactor."""
        filename = f"./optimal_angles/data_signal_(freq,psi0,varSens,fixedSens)--{file_tag}.npy"
        if skip_if_exists and os.path.exists(filename):
            return         
        freq_List = np.logspace(
            np.log10(freq_tuple[0]), np.log10(freq_tuple[1]), num=freq_tuple[2]
        ...
        def point_log_sens_given_psi0(psi0_vec, *params):
            ...
            return self.point_log_sens_given_angles([psi0_vec[0], pi / 2, 0], *params, **kwargs)
        ...
        def signal_varRO_given_freq(freq):
            args0 = (freq, xRatio0, *losses1, pumpPhi0)
            result = dual_annealing(
                point_log_sens_given_psi0, bounds=bounds1, args=args0, maxiter=max_iter
            )
            return result.x

        psi0_List = np.array(p_map(signal_varRO_given_freq, freq_List))

        fixed_signal_sens = self.sensList_vs_freq(
            [xRatio0, *losses1, *signalRO], freq_tuple, **kwargs
        )
        ...
        def sens_signal_varRO_given_freq(f):
            freq_index = np.searchsorted(freq_List, f)
            return self.ASDSh(f, *params0, psi0_List[freq_index], pi / 2, 0, **kwargs)
    ...
    def optimal_angles_given_freq(
        self, freq, xRatio, losses1, pumpPhi=pumpPhi0, max_iter=5, initial_guess=None, **kwargs
    ):
        """**kwargs: Options passed to ASDSh, e.g. extSqzFactor.
        See documentation at <https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.dual_annealing.html>."""
        bounds1 = ((0, 2 * pi), (0, 2 * pi), (0, 2 * pi), (0, 2 * pi))
        args0 = (freq, xRatio, *losses1, pumpPhi)
        def point_log_sens_given_angles_complex_with_kwargs_set(psi_angles_complex, *params):
            return self.point_log_sens_given_angles_complex(psi_angles_complex, *params, **kwargs)
        
        result = dual_annealing(
            point_log_sens_given_angles_complex_with_kwargs_set,
            bounds=bounds1,
            args=args0,
            maxiter=max_iter,
            x0=initial_guess,
    ...
    def optimal_sens_given_freq(
        self, freq, xRatio, losses1, pumpPhi=pumpPhi0, max_iter=5, **kwargs
    ):
        """**kwargs: Options passed to ASDSh, e.g. extSqzFactor."""
        bounds1 = ((0, 2 * pi), (0, 2 * pi), (0, 2 * pi), (0, 2 * pi))
        args0 = (freq, xRatio, *losses1, pumpPhi)
        ...
        return self.ASDSh(
            freq, xRatio, *losses1, pumpPhi, *angles[:-1], psi3=angles[-1], **kwargs
        )

    def save_optimal_filter(self, freq_tuple, losses1, file_tag, max_iter=5, skip_if_exists=False, freq_List=None, initial_guess_array=None, **kwargs):
        ...
        **kwargs: Options passed to ASDSh, e.g. extSqzFactor."""
        filename = f"./optimal_angles/data_optimal_(freq,angles_complex,sens)--{file_tag}.npy"
        if skip_if_exists and os.path.exists(filename):
            return
        ...
        # bounds1 = ((0,2*pi), (0,2*pi), (0,2*pi), (0,2*pi))
        global global_min_w_complex_given_freq_index
        def global_min_w_complex_given_freq_index(freq_index):
            freq = freq_List[freq_index]
            if initial_guess_array is not None:
                initial_guess = initial_guess_array[freq_index]
            else:
                initial_guess = None
            return self.optimal_angles_given_freq(
                freq, xRatio0, losses1, max_iter=max_iter, initial_guess=initial_guess, **kwargs
            )

        angles_complex_List = np.array(
            p_map(global_min_w_complex_given_freq_index, range(len(freq_List)))
        )
        ...
        sens_varRO_complex_List = np.array(
            p_map(sens_w_variationalRO_complex, freq_List)
        )

        data_set = np.empty((len(freq_List), 6))
    ...
    def save_optimal_filter_no_variational(
        self, freq_tuple, losses1, file_tag, max_iter=5, skip_if_exists=False, **kwargs
    ):
        """saves the optimal filter only changing psi2 and psi3 between fmin and fmax
        freq_tuple = (fmin, fmax, fnum), losses1 like losses0, file_tag for path
        **kwargs: Options passed to ASDSh, e.g. extSqzFactor."""
        filename = f"./optimal_angles/data_optimal_no_var_(freq,psi2,psi3,sens)--{file_tag}.npy"
        if skip_if_exists and os.path.exists(filename):
            return        
        freq_List = np.logspace(
            np.log10(freq_tuple[0]), np.log10(freq_tuple[1]), num=freq_tuple[2]
        ...
        def _point_log_sens_given_psi23(psi23, *params):
            return np.log10(self.ASDSh(*params, psi23[0], psi3=psi23[1], **kwargs))
        ...
        def _optimal_psi23_given_freq(freq, xRatio, losses1, max_iter=5):
            ...
            args0 = (
                ...
                *losses1,
                ...
                pi / 2,
                pumpPhi0,
            )  # signal and idler at sig. response max
            result = dual_annealing(
                _point_log_sens_given_psi23,
                bounds=bounds1,
                args=args0,
                maxiter=max_iter,
            )
        ...
        def sens_w_psi23(f):
            freq_index = np.searchsorted(freq_List, f)
            return self.ASDSh(
                f, *params0, psi23_List[freq_index, 0], psi3=psi23_List[freq_index, 1], **kwargs
            )

        sens_psi23_List = np.array(p_map(sens_w_psi23, freq_List))

        data_set = np.empty((freq_tuple[2], 4))
        data_set[:, 0], data_set[:, 1:3], data_set[:, 3] = (
            freq_List,
            psi23_List,
            sens_psi23_List,
        )
    ...
    def dIS_ASDSx(
        ...
        """degenerate internal squeezing, ASD of Sx, using fn from mathematica
        extSqzFactor = 1 is no external squeezing, 1/10 is 10dB injected external squeezing etc."""
        ga = self.ga_fn(T_loss_a)
        gbtot = self.gbtot_fn(T_loss_b)
        x = xRatio * self.dIS_singularity_thr(ga, gbtot)
    ...
    def dIS_sigTabs(
        ...
        """degenerate internal squeezing, abs of signal transfer function, using fn from mathematica
        extSqzFactor = 1 is no external squeezing, 1/10 is 10dB injected external squeezing etc."""
        ga = self.ga_fn(T_loss_a)
        gbtot = self.gbtot_fn(T_loss_b)
        x = xRatio * self.dIS_singularity_thr(ga, gbtot)
    ...
    def dIS_plot_NSR_vs_freq(
        ...
        for i, params in enumerate(paramsList):
            ...
            with Pool() as pool:
                ax.loglog(
                    f_List,
                    pool.map(_sens_given_params, f_List),
                    fmt_List[i],
                    label=labels[i],
                )
...
def extSqzFactor_given_dB_reduction(dB_reduction : float) -> float:
...
def parallel_map(
    ...
    Behaviour with and without display_progress_bar is sometimes different. TODO: find out why ptqdm acts this way.
